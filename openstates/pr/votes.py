...
def retrieve(url):
    filename, extension = get_filename_parts_from_url(url)
    filename+='.'+extension
    f = open(filename, 'wb')
    c = pycurl.Curl()
...
def document_to_html(file_path):
	...
	name_guid = os.path.splitext(file_path)[1]
	if '.pdf' == name_guid :
		warning("Pdf file found on file");

   	# convert the file, using a temporary file with the same filename
    command = "abiword -t %(tmp)s/%(name_guid)s.html %(file_path)s; cat %(tmp)s/%(name_guid)s.html" % locals()    
...
def wget_doc(url_path):
	print(self.document_to_html(self.retrieve(url_path)))



class PRVoteScraper(VoteScraper):
    state = 'pr'
