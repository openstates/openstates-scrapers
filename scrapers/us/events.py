...
class USEventScraper(Scraper, LXMLMixin):
    ...
    def scrape_senate(self):
        ...
        for row in rows:
            ...
            try:
                event_date = datetime.datetime.strptime(row.xpath("string(date)"), "%d-%b-%Y %H:%M %p")
            except ValueError:
                event_date = datetime.datetime.strptime(row.xpath("string(date)"), "%d-%b-%Y")
    ...
    def house_meeting(self, xml, source_url):
        ...
        start_dt = datetime.datetime.strptime("{} {}".format(meeting_date, start_time), "%Y-%m-%d %H:%M:%S")
        ...
        if end_time != "":
            end_dt = datetime.datetime.strptime("{} {}".format(meeting_date, end_time), "%Y-%m-%d %H:%M:%S")
        ...
        for com in coms:
            ...
            event.add_participant(
                com_name, type="committee", note="host",
            )
