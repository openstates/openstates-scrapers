from spatula import HtmlPage,CSS,URL, HtmlListPage, SelectorError, XPath
...
class HouseCommitteeDetail(HtmlPage):
    ...
    def process_page(self):
        com = self.input
        try :
            Chair_member = CSS("#ctl00_ContentPlaceHolder1_lnkChair").match_one(self.root).getchildren()[0].tail.strip().replace("Rep.","")
            VC_member = CSS("#ctl00_ContentPlaceHolder1_lnkViceChair").match_one(self.root).getchildren()[0].tail.replace("Rep.","")
            role_C = CSS("#ctl00_ContentPlaceHolder1_lnkChair").match_one(self.root).text_content()[:5].strip().replace("Rep.","")
            role_VC = CSS("#ctl00_ContentPlaceHolder1_lnkViceChair").match_one(self.root).text_content()[:10].strip().replace("Rep.","")
            com.add_member(Chair_member, role_C)
            com.add_member(VC_member,role_VC)
        except SelectorError:
            CoChair_member = CSS("#ctl00_ContentPlaceHolder1_lnkChair").match_one(self.root).getchildren()[0].tail.strip().replace("Rep.", "")
            role_CoC = CSS("#ctl00_ContentPlaceHolder1_lnkChair").match_one(self.root).text_content()[:10].strip().replace("Rep.", "")
