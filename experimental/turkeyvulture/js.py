...
<div>
{% if person.length %}
<h2>Legislators ({{ person.length }})</h2>
<ul>
{% for leg in person %}
    <li>
    <div class='content'>
    <i class='icon-user'></i>
    <h3>
        <a href='http://openstates.org/{{leg.state}}/legislators/{{leg._id}}/'>{{ leg.full_name }}</a>
...
{% if committee %}
<h2>Committees ({{ committee.length }})</h2>
<ul>
{% for c in committee %}
    <div class='content'>
    <i class='icon-lock'></i>
