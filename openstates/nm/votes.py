...
def validate_house_vote(row_headers, sane_row, vote_record):
    ...
    sorted_row_header = sorted(row_headers.items(), key=operator.itemgetter(0))
    start_count = -1
...
def sane_matches_captured(sane, vote_record):
    return (
        sane["yes"] == len(vote_record["yes"])
        and sane["no"] == len(vote_record["no"])
        and sane["excused"] == len(vote_record["excused"])
        and sane["absent"] == len(vote_record["absent"])
        and sane["other"] == len(vote_record["other"])
    )
            ...
            if t_begin < row_x <= t_stop:
                ...
                for x in range(0, len(cells), 2):
                    ...
                    if cells[x + 1][1] not in column_map:
                        ...
                        column_map[cells[x + 1][1]] = match_header(
                            list(row_heads.keys()), cells[x + 1][1] + cells[x + 1][2]
                        )
