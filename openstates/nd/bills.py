...
class NDBillScraper(BillScraper):
    ...
    def scrape_bill_actions(self, assembly_url, bill_number, session):
        """
    ...
    def scrape_bill_sponsors(self, assembly_url):
        """
        Scrape a list of all bill sponsors for a given year and chamber.

        Sponsors can not be accurately scraped from the bill details page,
        so instead they are scraped from an index by member/committee.

        Each sponsor is buffered into a list which is set as the value in
        ...
        attributes.
        """
        url = self.site_root + assembly_url + '/sponsor-inquiry/index.html'
