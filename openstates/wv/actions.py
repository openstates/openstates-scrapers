...
rules = (
    ...
    Rule([', but first to .+?; then (?P<committees>[^;]+)',
    ...
    Rule('Senator (?P<legislators>.+? )requests '
         'to be removed as sponsor of bill'),
    ...
    Rule(['To .+? then (?P<committees>.+) then',
          'reading to (?P<committees>.+)']),
    ...
    Rule(['(?i)originating in (House|Senate) (?P<committees>.+)',
