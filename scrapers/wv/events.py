...
class WVEventScraper(Scraper, LXMLMixin):
    ...
    bill_regex = r"((House Bill|Senate Bill|[HS]\.\s*[BCR]\.\s*([R]\.)*|H\.\s*B\.)\s*\d+)"
    ...
    def scrape_committees(self, url):
        ...
        for link in page.xpath('//div[@id="wrapleftcol"]/a[contains(@href,"agendas.cfm")]/@href'):
    ...
    def scrape_meeting_page(self, url):
        ...
        where = page.xpath('//div[@id="wrapleftcol"]/b[contains(text(), "Location")]/text()')[0].strip()
        desc = page.xpath('//div[@id="wrapleftcol"]/blockquote[1]')[0].text_content().strip()
        ...
        for row in page.xpath('//div[@id="wrapleftcol"]/blockquote[1]/p'):
            if row.text_content().strip() != '':
                ...
                for bill in re.findall(self.bill_regex, row.text_content()):
                    bill_id = re.sub("\.\s*", "", bill[0], flags=re.IGNORECASE)
                    bill_id = re.sub("house bill", "HB", bill_id, flags=re.IGNORECASE)
                    bill_id = re.sub("senate bill", "SB", bill_id, flags=re.IGNORECASE)
