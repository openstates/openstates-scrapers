...
def build_legislators(cursor):
    ...
    cursor.execute("SELECT PersonID, Employeeno, FirstName, LastName, MiddleName,"
                   "LegislativeBody, District FROM Legislators")
...
def legislator_name(legislator):
    ...
    return ' '.join(filter(None, [legislator['FirstName'].strip(),
                                  legislator['MiddleName'].strip(),
                                  legislator['LastName'].strip()]))
