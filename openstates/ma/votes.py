...
class MAVoteScraper(VoteScraper):
    ...
    def scrape_bill(self, url, chamber, session):
        ...
        for bill_no in bill_urls:
            ...
            if len(dates)>0:
                motions = bill_page.xpath("//div[@class='table-responsive']/table/tbody/tr/td[@class='titleCol']/text()")
                #data for yes and no counts
                first = bill_page.xpath("//div[@class='table-responsive']/table/tbody/tr/td[@class='statusCol']/div/text()")
                #data for other counts
                second = bill_page.xpath("//div[@class='table-responsive']/table/tbody/tr/td[@class='statusCol']/text()")
                ...
                for i in xrange(1,len(second),2):
                    other_counts.append(map(int, re.findall(r'\d+', "".join((second[i].replace('\r\n','').split())))))

                for i in range(0,len(dates)):
                    vote = BillyVote(chamber, dates[i], motions[i], y_n_counts[i][0]>y_n_counts[i][1], y_n_counts[i][0], y_n_counts[i][1], other_counts[i][0] )
                    vote.add_source(url)
